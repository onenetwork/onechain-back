...
contract('Backchain', function(accounts) {
  ...
  it("should be initialized with a orchestrator and no hashes", function() {
    ...
    return Backchain.deployed().then(function(instance) {
      ...
      assert.equal(hashCount.toNumber(), 0, "Backchain should initially be empty");
      return bk.orchestrator.call();
    }).then(function(orchestrator) {
      assert.equal(orchestrator.toString(), accounts[0], "Backchain should be initialized with an orchestraotr");
      assert.notEqual(orchestrator.toString(), accounts[1], "Backchain should be initialized with a orchestrator");
  ...
  it("should accept posts from the orchestrator", function() {
    ...
    return Backchain.deployed().then(function(instance) {
      ...
      assert.equal(hashCount.toNumber(), 1, "Backchain should have added one hash");
